...
function initDOMEnhancements() {
    ...
    // showToast → 居中通知 (不再重定义, 直接路由到 showCenteredMessage)
    window.showToast = function (msg, type, dur) {
        showCenteredMessage(msg, type || 'info', dur || 3500);
    };
}
...
function upgradeEmptyStates() {
    document.querySelectorAll('.empty-state, [data-empty]').forEach(el => {
        ...
        // 静态 SVG + 转义文本, 无用户可控脚本, CSP 安全
        // text 通过 textContent 设置, 避免 XSS
        const descHtml = desc ? '<p class="el-empty-desc">' + escapeHtml(desc) + '</p>' : '';
        const wrapper = document.createElement('div');
        wrapper.className = 'el-empty-wrapper';
        wrapper.innerHTML =
            '<div class="el-empty-image">' +
                ...
                '</svg>' +
            '</div>' +
            '<p class="el-empty-text"></p>' +
            descHtml;

        // 通过 textContent 设置文本, 防止 XSS (即使 text 含 <script> 也不会执行)
        const textEl = wrapper.querySelector('.el-empty-text');
