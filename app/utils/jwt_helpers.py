...
def is_token_revoked(jti: str) -> bool:
    ...
    return db.session.execute(db.select(RevokedToken.id).filter_by(jti=jti).limit(1)).scalar() is not None
...
def cleanup_expired_revocations():
    ...
    try:
        db.session.execute(db.delete(RevokedToken).where(RevokedToken.expires_at < datetime.now(UTC)))
        db.session.commit()
    except Exception:
        db.session.rollback()
...
def _get_jwt_secret() -> str:
    ...
    if not secret:
        raise RuntimeError('JWT_SECRET_KEY 未配置。生产环境必须在 .env 中设置 JWT_SECRET_KEY。')
    return secret
...
def generate_token_pair(user_id: int, username: str, role: str, token_version: int = 1) -> dict[str, str]:
    """
