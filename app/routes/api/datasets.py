          ...
          schema:
            ...
            properties:
              ...
              file:
                type: string
                format: binary
                description: CSV/Excel/JSON/Parquet 文件
...
def auto_config(dataset_id):
    ...
    responses:
      200:
        description: "训练配置 (ml_task_type, algorithm, framework, target_column, test_size, total_epochs, param_presets 等)"
      400:
        description: 分析失败
      404:
        description: 数据集不存在
    ...
    result = DatasetRecommendationService.recommend(
        dataset.file_path, target_col, dataset.file_format, known_n_samples=dataset.row_count or None
    )
    ...
    CATEGORY_TASK_MAP = {
        ...
        'nlp': (
            'classification',
            '⚠ 此数据集为NLP文本类型，标准分类/回归可能不适用。建议使用TF-IDF+分类器或深度学习。',
        ),
        'vision': (
            'classification',
            '⚠ 此数据集为视觉类型，表格型算法不适用于图像特征。建议使用PyTorch/TensorFlow CNN。',
        ),
        'synthetic': (
            None,
            '⚠ 此数据集为合成/生成式数据，target列可能为随机合成值而非真实标签，模型可能无法学到有效模式。',
        ),
    ...
    forced_task, category_warning = CATEGORY_TASK_MAP.get(ds_category, (None, None))

    # 启发式检测
    column_names = analysis.get('column_names', [])
    latent_cols = [c for c in column_names if c.startswith('latent_')]
    is_latent_data = len(latent_cols) > 0 and len(latent_cols) >= len(column_names) * 0.7
    ...
    if is_nlp:
        ...
        # 替换推荐理由
        reason_text = (
            '[NLP Transformer] 使用预训练BERT模型进行迁移学习微调。'
    ...
    # --- 视觉专属: 强制 PyTorch 深度学习 ---
        ...
        total_epochs = 20
        reason_text = (
            '[Vision DL] 视觉特征使用PyTorch MLP深度网络训练。自动加宽隐藏层+增加dropout防过拟合。' + reason_text
        )
    else:
        # --- 框架 ---
...
def smart_params(dataset_id):
    ...
    responses:
      200:
        description: "推荐参数 (params, reason, confidence, tips, gridsearch_suggestion 等)"
      400:
        description: 分析失败
      404:
        description: 数据集不存在
        ...
        application/json:
          schema:
            ...
            properties:
              ...
              previous_metrics:
                type: object
                description: 上次训练的 final_metrics
              previous_params:
                type: object
                description: 上次使用的超参数
    responses:
      200:
        description: 改进参数 + diagnosis
      400:
        description: 分析失败
      404:
        description: 数据集不存在
        ...
        application/json:
          schema:
            ...
            properties:
              ...
              metrics:
                type: object
                description: "训练指标 (如 {test_accuracy: 0.55})"
    responses:
      200:
        description: "诊断结果 (root_cause, confidence, explanation, suggested_actions)"
      400:
        description: 分析失败
      404:
        description: 数据集不存在
